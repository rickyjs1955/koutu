...
class SecurityVectors {
  ...
  static getDDoSVectors(): SecurityTestVector[] {
    return [
      ...
      },
      {
        ...
        payload: this.createNestedObject(1000),
        shouldBlock: true,
        attackType: 'dos'
      },
      {
  ...
  private static createNestedObject(depth: number): any {
    if (depth === 0) return { value: 'attack' };
...
class SecurityUtils {
  ...
  static createTimingAttackPayloads(): Array<{ name: string; payload: any; expectedTiming: 'fast' | 'slow' }> {
    return [
      {
...
class SecurityMockManager {
  ...
  static setup() {
    ...
    this._mocks.sanitization.sanitizeUrl.mockImplementation((url: string) => {
    ...
    this._mocks.sanitization.sanitizeUserInput.mockImplementation((input: string) => {
    ...
    this._mocks.uuid.mockReturnValue('test-uuid-12345' as any);
  }
  ...
  static setupVulnerableMocks() {
    // Setup mocks that simulate vulnerable behavior
    this._mocks!.sanitization.sanitizeUrl.mockImplementation((url: string) => url); // No sanitization
    this._mocks!.sanitization.sanitizeUserInput.mockImplementation((input: string) => input); // No sanitization
  }

  static setupSecureMocks() {
    this.setup(); // Reset to secure defaults
  }

  static setupTimingVulnerability() {
    // Simulate timing attack vulnerability
    this._mocks!.authService.ensureMinimumResponseTime.mockImplementation(async (startTime: number, minTime: number) => {
...
describe('OAuthController Security Test Suite', () => {
  ...
  describe('XSS Attack Prevention', () => {
    ...
    it('should handle XSS in HTTP headers', async () => {
      ...
      expect(context.res.redirect).toHaveBeenCalledWith(
    ...
    it('should prevent DOM-based XSS in callback URLs', async () => {
      ...
      SecurityMockManager.oauth.exchangeCodeForTokens.mockResolvedValue({
        access_token: 'token',
        token_type: 'Bearer',
        expires_in: 3600,
      ...
      SecurityMockManager.oauth.getUserInfo.mockResolvedValue({
        id: 'user123',
        email: 'user@example.com'
      });
      SecurityMockManager.oauth.findOrCreateUser.mockResolvedValue({
        id: 'user123',
        email: 'user@example.com'
      });
      SecurityMockManager.oauth.generateToken.mockReturnValue('jwt-token');
  ...
  describe('CSRF Attack Prevention', () => {
    test.each(SecurityVectors.getCSRFVectors())(
      ...
      async ({ payload, shouldBlock }) => {
        if (payload.useOldState) {
          ...
          // Simulate stored state
          const oauthStates = (global as any).oauthStates || {};
          oauthStates['test-uuid-12345'] = { createdAt: Date.now() };
    ...
    it('should generate cryptographically secure state parameters', async () => {
      ...
      SecurityMockManager.crypto.randomBytes = mockRandomBytes;

      const states = new Set<string>();
      
      for (let i = 0; i < 100; i++) {
        SecurityMockManager.uuid.mockReturnValue(`uuid-${i}` as any);
  ...
  describe('Timing Attack Prevention', () => {
    ...
    it('should have consistent timing for state validation', async () => {
      const validStateTiming = await SecurityUtils.measureExecutionTime(async () => {
        ...
        const oauthStates = (global as any).oauthStates || {};
        oauthStates['valid-state'] = { createdAt: Date.now() };
    ...
    it('should use ensureMinimumResponseTime consistently', async () => {
      ...
      for (const getOperation of operations) {
        ...
        expect(SecurityMockManager.auth.ensureMinimumResponseTime).toHaveBeenCalled();
      }
    });
  });
  ...
  describe('Authorization Bypass Prevention', () => {
    ...
    it('should prevent account takeover via email collision', async () => {
      ...
      SecurityMockManager.oauth.findOrCreateUser.mockRejectedValue(
  ...
  describe('DoS Attack Prevention', () => {
    ...
    test.each(SecurityUtils.createRateLimitTestData())(
      ...
      async ({ concurrent, sequential, expectBlocked }) => {
  ...
  describe('Information Disclosure Prevention', () => {
    test.each(SecurityVectors.getInformationDisclosureVectors())(
      ...
      async ({ payload }) => {
        ...
        const context = SecurityUtils.createSecurityContext({
          req: {
            params: { provider: 'google' },
            query: payload.triggerError ? { state: 'test-uuid-12345', code: 'auth-code' } : {},
            headers: {}
          }
        });
    ...
    it('should sanitize error messages in production', async () => {
      ...
      try {
        ...
        const error = (context.next as jest.Mock).mock.calls[0][0];
        expect(error).toBeDefined();
        expect(error.message).toContain('secretPassword123!'); // Raw error in test
      } finally {
    ...
    it('should not leak user enumeration through timing', async () => {
      ...
      const existingUserTiming = await SecurityUtils.measureExecutionTime(async () => {
        SecurityMockManager.reset();
        SecurityMockManager.auth.getUserAuthStats.mockResolvedValue({
          userId: 'user123',
      ...
      const nonExistentUserTiming = await SecurityUtils.measureExecutionTime(async () => {
        SecurityMockManager.reset();
        SecurityMockManager.auth.getUserAuthStats.mockRejectedValue(
  ...
  describe('OAuth Protocol Security', () => {
    ...
    it('should prevent token substitution attacks', async () => {
      ...
      SecurityMockManager.oauth.findOrCreateUser.mockImplementation(async (provider, userInfo) => {
    ...
    it('should prevent scope escalation attacks', async () => {
      ...
      if (SecurityMockManager.oauth.exchangeCodeForTokens.mock.calls.length > 0) {
  ...
  describe('Session Security', () => {
    it('should prevent session fixation attacks', async () => {
      ...
      expect(context.res.redirect).toHaveBeenCalledWith(
        expect.not.stringMatching(new RegExp(fixedSessionId))
      );
    });
    ...
    it('should handle concurrent sessions securely', async () => {
      ...
      sessionIds.forEach((sessionId, index) => {
        SecurityMockManager.uuid.mockReturnValueOnce(`uuid-${index}` as any);
    ...
    it('should prevent session hijacking via token theft', async () => {
      ...
      SecurityMockManager.oauth.generateToken.mockReturnValue(stolenToken);
      SecurityMockManager.oauth.exchangeCodeForTokens.mockResolvedValue({
    ...
    it('should invalidate sessions on security events', async () => {
      ...
      let ensureMinimumResponseTimeCalls = 0;
      
      // Create a simple mock that just counts calls
      SecurityMockManager.auth.ensureMinimumResponseTime.mockImplementation(async () => {
        ensureMinimumResponseTimeCalls++;
        return Promise.resolve();
      });
      
      for (let i = 0; i < failedAttempts; i++) {
        // Don't reset here to preserve call count
        const context = SecurityUtils.createSecurityContext({
          req: {
            params: { provider: 'google' },
      ...
      // After multiple failures, should maintain security
      expect(ensureMinimumResponseTimeCalls).toBeGreaterThanOrEqual(failedAttempts);
    });
  });
  ...
  describe('Comprehensive Security Integration', () => {
    ...
    it('should pass comprehensive security audit', async () => {
      ...
      SecurityMockManager.sanitize.sanitizeUrl.mockReturnValue('safe-url');
  ...
  describe('Performance Security', () => {
    it('should prevent algorithmic complexity attacks', async () => {
      const complexityAttacks = [
        ...
        {
          name: 'Large array',
          payload: new Array(10000).fill('attack')
        },
        {
      ...
      for (const attack of complexityAttacks) {
        ...
        const context = SecurityUtils.createSecurityContext({
          req: {
            params: { provider: attack.payload },
            query: {},
            headers: {}
          }
        });
